                    ...
                    <div className="w-full h-full bg-blue-200 smd:rounded-tl-none esm:rounded-tl-2xl smd:rounded-tr-2xl px-4 pt-10 smd:pt-4 flex flex-col gap-3">
                        ...
                        <div className="w-full h-full overflow-scroll hideScroll flex flex-col gap-3 pb-3">
                            {filteredTasks.map(task => (
                                ...
                                <div className={task.completed ? `relative w-full bg-green-50 rounded-md p-3 pr-6 shadow-sm hover:shadow-md transition duration-300 ` : `relative w-full bg-white rounded-md p-3 pr-6 shadow-sm hover:shadow-md transition duration-300 `} key={task.id}>
                                    <span className="relative flex h-3 w-3 mb-2">
                                        <span className={`${task.priority === 'High' && ' animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75'} ${task.priority === 'Low' && 'absolute h-full w-full bg-green-400 rounded-full'} ${task.priority === 'Medium' && 'absolute h-full w-full bg-blue-500 rounded-full'}`}></span>
                                        <span className={`${task.priority === 'High' && ' relative inline-flex rounded-full h-3 w-3 bg-red-500'}`}></span>
                                    </span>
